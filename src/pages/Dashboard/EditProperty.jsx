...
export default function EditProperty() {
  ...
  const { register, handleSubmit, trigger, formState: { errors }, watch, reset } = useForm({
    ...
    <div className="min-h-screen bg-[#fbf9f8] pt-[72px]">
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <nav className="flex items-center gap-1 text-sm text-[#717a6d] mb-6">
          ...
          <span className="text-[#41493e]">Edit Listing</span>
        </nav>

        <h1 className="font-['Playfair_Display'] text-3xl font-bold text-[#1b1c1c] mb-2">Edit Property</h1>
        <p className="text-sm text-[#41493e] mb-8">Update your property listing details.</p>
              ...
              <>
                ...
                <div>
                  ...
                  {existingImages.length > 0 && (
                    <div className="mb-3">
                      <p className="text-xs text-[#41493e] mb-2">Current images:</p>
                      <div className="flex gap-2 flex-wrap">
                        {existingImages.map((src, i) => (
                          <div key={i} className="relative w-20 h-20 rounded-xl overflow-hidden border border-[#c0c9bb]">
                            <img src={src} alt="" className="w-full h-full object-cover" />
                            <button type="button" onClick={() => removeExistingImage(i)}
                              className="absolute top-1 right-1 w-5 h-5 bg-[#ba1a1a] text-white rounded-full flex items-center justify-center text-xs hover:bg-[#93000a]">
                              ×
                            </button>
                          </div>
                        ))}
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-[#c0c9bb] rounded-xl cursor-pointer hover:border-[#1b5e20] hover:bg-[#e8f5e9]/30 transition-colors">
                    ...
                    <span className="text-sm text-[#41493e] mt-1">Click to upload new photos</span>
            ...
            {step === 3 && (
              <>
                ...
                <div className="bg-[#e8f5e9] border border-[#91d78a] rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#1b5e20]">Changes will be saved</p>
                    <p className="text-xs text-[#1b5e20]/80 mt-0.5">
                      Your updated listing will be visible immediately if already approved.
                    </p>
                  </div>
                </div>
                <div className="space-y-2 text-sm text-[#41493e]">
                  ...
                  <div className="flex justify-between py-2">
                    ...
                    <span>{existingImages.length + images.length} total</span>
                  </div>
                </div>
              </>
            )}
